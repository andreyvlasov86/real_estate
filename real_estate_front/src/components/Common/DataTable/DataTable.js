...
class DataTableView extends Component {
    ...
    componentDidMount() {
        ...
        if (this.props.func_config && this.props.func_config.SingleColumnSearch && this.props.func_config.SingleColumnSearch.targets === 'all') {
        ...
        //Regex Search
        if (this.props.func_config && this.props.func_config.regexSearch) {
        ...
        //Single Search
        if (this.props.func_config.SingleColumnSearch && this.props.func_config.SingleColumnSearch.target) {
            const column = this.props.func_config.SingleColumnSearch.target
            console.log('DATATABLES target search', column)
            function filterSingle (column) {
                table
                    .column(7)
                    .search(
                        $('#custom_filter').val(),
            ...
            $('#custom_filter').on( 'keyup change', function () {
                filterSingle();
            } );
        }
    ...
    shouldComponentUpdate(nextProps) {
        console.log('shouldComponentUpdate', nextProps.tableActions)

        if(nextProps.data.token.access_token !== this.props.data.token.access_token) {
            $('#DataTable').DataTable().ajax.reload(null, false);
        }

        if(nextProps.tableActions && nextProps.tableActions.reload) {
            console.log('Reload table', nextProps.tableActions.reload)
            $('#DataTable').DataTable().ajax.reload(null, false);
        }
    ...
    render() {
        return (
            <div>
                {
                    this.props.func_config.SingleColumnSearch && this.props.func_config.SingleColumnSearch.target &&
                    <div style={{float: 'right'}}>
                        Text Search: <input type="text" className="custom_filter" id="custom_filter" />
                    </div>
